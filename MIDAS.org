...
** Password security
   + Run the httpd server
     ...
     mhttpd
     #+END_SRC
   + Looks like MIDAS is more sensible with security now. We're running behind a firewall so it's all good.
...
** Logger
   ...
   + Set up MIDAS to make sure mlogger is always running
     ...
     4. Set Required to "Yes", Start Commant to "mlogger -D", Alarm class to "Alarm"
